...
exports.getAvailableHost = function(route, request) {
  ...
  var 
    ...
    ,host
  	...
  	,session = sessionRoute[request.client.remoteAddress] = sessionRoute[request.client.remoteAddress] || {
			lastAccess : 0,
			lastServer : null
		}
	;
...
exports.failed = function(route) {	
	var routeStatus = routeStatus(route);
	
	routeStatus.failures++
};

exports.served = function(route, host, bytes) {	
	var thisRoute = routeStatus(route);
	
	thisRoute.hosts[host.route] = (thisRoute.hosts[host.route])?thisRoute.hosts[host.route]:{
		successes : 0,
		failures : 0,
		bytes : 0
	}
	
	thisRoute.hosts[host.route].successes++
	thisRoute.hosts[host.route].bytes += bytes
